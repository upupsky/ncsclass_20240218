...
![打卡1](./创新班训练营教学内容.assets/打卡1.png)

打卡流程示意图。

![打卡](./创新班训练营教学内容.assets/打卡.png)

## 1.4 E53底板的基本介绍

![底板介绍](./创新班训练营教学内容.assets/底板介绍.png)
...
| 章节   | 章名               | 主要内容                                                     |
| ------ | ------------------ | ------------------------------------------------------------ |
| 第1章  | 文档撰写规范       | word文档撰写排版规范                                         |
| 第2章  | 文件夹命名规范     | 文件夹命名规范                                               |
| 第3章  | 文件命名规范       | 文件命名规范                                                 |
| 第4章  | Git版本管理规范    | 仓库命名规范<br />嵌入式软件分支规范<br />系统软件分支规范   |
| 第5章  | 软件命名规范       | 软件命名规范                                                 |
| 第6章  | C编程规范          | 概述<br />命名规范：源文件、头文件、宏/枚举、变量、结构体与联合体、函数<br />内容排版与注释：源文件、头文件 |
| 第7章  | C++编程规范        | 概述<br />类命名规范<br />                                   |
| 第8章  | Java编程规范       | 类命名规范<br />抽象类命名<br />接口命名<br />方法命名<br />属性命名 |
| 第9章  | Kot编程规范        | 类命名规范<br />抽象类命名<br />接口命名<br />方法命名<br />属性命名 |
| 第10章 | JavaScript编程规范 | 类命名<br />函数<br />Vue视图文件<br />变量命名<br />常量    |
...
![](D:\work\西南财经大学天府学院\2025年春季学期\创新班相关资料\教学内容\training\创新班训练营教学内容.assets\PCB制作过程.png)

### 3.1.2 原理图

![](D:\work\西南财经大学天府学院\2025年春季学期\创新班相关资料\教学内容\training\创新班训练营教学内容.assets\底板原理图_00.png)

### 3.1.3 PCB及PCB3D模型

![](D:\work\西南财经大学天府学院\2025年春季学期\创新班相关资料\教学内容\training\创新班训练营教学内容.assets\PCB.png)
...
![](D:\work\西南财经大学天府学院\2025年春季学期\创新班相关资料\教学内容\training\创新班训练营教学内容.assets\安装软件.png)

### 3.2.2 开发环境安装流程

![](D:\work\西南财经大学天府学院\2025年春季学期\创新班相关资料\教学内容\training\创新班训练营教学内容.assets\开发环境搭建流程.png)
...
![](D:\work\西南财经大学天府学院\2025年春季学期\创新班相关资料\教学内容\training\创新班训练营教学内容.assets\Blinky.png)

### 3.3.2 工程示例

![](D:\work\西南财经大学天府学院\2025年春季学期\创新班相关资料\教学内容\training\创新班训练营教学内容.assets\Blinky示例.png)

## 3.4 Button示例运行

参考Zephyr的工程示例https://docs.zephyrproject.org/3.5.0/samples/basic/button/README.html。一些基础工程NCS不包括在其Samples中，直接参考Zephyr工程示例即可，内附有详细的工程使用说明。

### 3.4.1 理论知识学习

![](D:\work\西南财经大学天府学院\2025年春季学期\创新班相关资料\教学内容\training\创新班训练营教学内容.assets\button.png)

## 3.4.2 工程示例

![](D:\work\西南财经大学天府学院\2025年春季学期\创新班相关资料\教学内容\training\创新班训练营教学内容.assets\button示例.png)
...
在evb上实现LED0的闪烁，并尝试添加LED1以100ms周期性闪烁。
...
这段代码是设备树（Device Tree）中的一个节点，描述了一个名为 `gpiote0` 的 GPIOTE（GPIO Task and Event）设备。GPIOTE 是 Nordic Semiconductor nRF 系列微控制器中的一个模块，用于处理基于 GPIO 的事件和任务。以下是对该代码段中各个部分的理解：

- `gpiote: gpiote0: gpiote@40006000 {`：这定义了一个设备树节点，其中 `gpiote` 是节点名的根，`gpiote0` 是节点标签，用于在其他地方引用这个节点，`@40006000` 表示该设备的基地址是 0x40006000。
- `compatible = "nordic,nrf-gpiote";`：这个属性指定了设备的兼容性字符串，它告诉设备树解析器这个节点描述的是一个与 “nordic,nrf-gpiote” 兼容的设备。这通常用于匹配正确的设备驱动程序。
- `reg = <0x40006000 0x1000>;`：这个属性定义了设备的寄存器映射。`0x40006000` 是设备的基地址，而 `0x1000` 是映射的长度（大小），这意味着设备占用从 0x40006000 到 0x40006000 + 0x1000 - 1 的地址空间。
- `interrupts = <6 5>;`：这个属性指定了设备使用的中断。在 Nordic nRF 系列中，中断通常由一个中断号和一个优先级组成。在这里，`6` 是中断号，而 `5` 是分配给该中断的优先级。
- `status = "disabled";`：这个属性表示设备的初始状态是禁用的。这意味着在系统启动时，GPIOTE 设备不会被自动启用，需要在运行时通过软件来启用它。
- `instance = <0>;`：这个属性指定了设备的实例编号。在具有多个相同类型设备的情况下，每个设备都会有一个唯一的实例编号。在这里，`0` 表示这是第一个（或唯一的）GPIOTE 实例。

总的来说，这个节点描述了 Nordic nRF 微控制器上的一个 GPIOTE 模块，提供了其寄存器映射、中断信息、兼容性字符串、状态和实例编号。这样的描述允许操作系统或固件在启动时识别并配置这个硬件模块。
