...
同时设置path环境变量，例如：D:\soft\qemu（根据自己的安装路径设置），如下：
...
>
> - 请仿真实现业务逻辑：手环充电过程中有2个与充电状态量。状态量1：插入充电器会芯片输出`1`，拔掉充电器芯片输出`0`。状态量2：充电中芯片输出`0`，充电完成或未充电芯片输出`1`。请结合真值表方法给出：手环充电过程中红灯快闪烁，充满绿灯常亮。（拓展：低电量时红灯满闪，蓝牙通信与定位关闭；电量充足时蓝牙通信与定位开启）
