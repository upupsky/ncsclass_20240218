...
>
>Tips：使用shell手动启停广播，也可以使用定时器启停广播！
>
>```
> static int cmd_start_adv(const struct shell *sh, size_t argc, char **argv)
> {
>	 int battery_percent = atoi(argv[1]);
>	 ad[0].data = &battery_percent;
>	 bt_le_adv_start(BT_LE_ADV_NCONN_IDENTITY, ad, ARRAY_SIZE(ad), sd, ARRAY_SIZE(sd));
>	 return 0;
> }
> SHELL_CMD_REGISTER(start, NULL, "start adv", cmd_start_adv); 
>```
>
>```
> static int cmd_stop_adv(const struct shell *sh, size_t argc, char **argv)
> {
>	 bt_le_adv_stop();
>	 return 0;
> }
> SHELL_CMD_REGISTER(stop, NULL, "stop adv", cmd_stop_adv); 
>```
...
> **本周练习**
>
> 请在当前从机工程`peripheral_hr`中将心率和电量两种数据添加到广播中，实现动态广播（附属幼儿园项目主要采用该方案）。
>
> Tips：启用定时器周期性更新广播数组，一般1秒比较合适，并合理调整广播的`interval_min ` 和`interval_max `实现低功耗。

```
1.修改(添加)厂商自定义广播代码[注意要去掉const修饰符]
...
#endif /* CONFIG_BT_EXT_ADV */
	BT_DATA_BYTES(BT_DATA_MANUFACTURER_DATA, 0xEE, 0xFF),
};
2.修改(添加)获取实时心率和电量
static uint8_t current_battery_level = 100U;
...
static uint8_t current_heart_rate = 90U;
...
while (1) {
		...
		bas_notify();
		// 更新心率和电量并开启广播
		char custom_adv_data[2] = {current_heart_rate, current_battery_level};
		ad[2].data_len = sizeof(custom_adv_data);
		ad[2].data = (const uint8_t *)custom_adv_data;
...
Zephyr 中线程是最基本的执行单位。通过 `k_thread_create()` 创建线程。
...
void main(void) {
    k_thread_create(&thread1_data, thread1_stack, K_THREAD_STACK_SIZEOF(thread1_stack),
                    thread1_entry, NULL, NULL, NULL, 5, 0, K_NO_WAIT);
    k_thread_create(&thread2_data, thread2_stack, K_THREAD_STACK_SIZEOF(thread2_stack),
                    thread2_entry, NULL, NULL, NULL, 5, 0, K_NO_WAIT);
}
```
...
void event_listener(void) {
    while (1) {
        uint32_t received = k_event_wait(&my_event, BIT_READY, false, K_FOREVER);
        if (received & BIT_READY) {
...
void event_sender(void) {
    k_sleep(K_SECONDS(2));
    k_event_post(&my_event, BIT_READY);
}
```

### FIFO（先进先出队列）

Zephyr 提供 `k_fifo` 作为线程之间传递数据的一种方式，常用于**生产者-消费者模型**。

- FIFO 内部可以传递结构体指针。
- 数据先进先出（First-In First-Out）。
- 线程间同步隐式完成（取数据线程会阻塞直到有数据）。
...
/* 定义一个结构体传递数据 */
struct data_item_t {
    void *fifo_reserved;  // 必须保留
    int value;
};

/* 生产者线程 */
void producer(void) {
    static struct data_item_t item;
    int count = 0;

    while (1) {
        item.value = count++;
        printk("Producer: sending value %d\n", item.value);
        k_fifo_put(&my_fifo, &item);  // 放入队列
        k_sleep(K_SECONDS(1));
    }
}
...
/* 消费者线程 */
void consumer(void) {
    struct data_item_t *rx_item;

    while (1) {
        rx_item = k_fifo_get(&my_fifo, K_FOREVER);  // 阻塞获取
        printk("Consumer: received value %d\n", rx_item->value);
    }
}

/* main 中创建线程 */
K_THREAD_STACK_DEFINE(prod_stack, 512);
K_THREAD_STACK_DEFINE(cons_stack, 512);
...
void main(void) {
    k_thread_create(&prod_thread, prod_stack, K_THREAD_STACK_SIZEOF(prod_stack),
                    (k_thread_entry_t)producer, NULL, NULL, NULL,
                    5, 0, K_NO_WAIT);
    k_thread_create(&cons_thread, cons_stack, K_THREAD_STACK_SIZEOF(cons_stack),
                    (k_thread_entry_t)consumer, NULL, NULL, NULL,
                    5, 0, K_NO_WAIT);
}
